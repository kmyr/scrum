...
function action(action, scrumfiles) {
    if (scrumfiles.argv && scrumfiles.argv.actions[action]) {
        return scrumfiles.argv.actions[action];
    }

    if (scrumfiles.local && scrumfiles.local.actions[action]) {
        return scrumfiles.local.actions[action];
    }

    if (scrumfiles.global && scrumfiles.global.actions[action]) {
        return scrumfiles.global.actions[action];
    }
